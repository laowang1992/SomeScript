...
p <- add_argument(parser = p, arg = "--tool", short = "-t", help = "alignment tool, 'mummer' or 'minimap2'", type = "character")
p <- add_argument(parser = p, arg = "--input", short = "-i", help = "coords file from mummer program 'show-coords' or paf file from minimap2", type="character")
...
# read in alignments
if (tool == "mummer") {
  alignments <- read_table(file = input_filename, skip = 5, col_names = F, col_types = "ddcddcddcdccc")
  # set column names
  alignments <- alignments %>% select(refStart = X1, refEnd = X2, queryStart = X4, queryEnd = X5, lenAlnRef = X7, lenAlnQuery = X8, pctIDY = X10, refID = X12, queryID = X13)
} else if (tool == "minimap2") {
  alignments <- read_tsv(file = input_filename, col_names = F, col_types = "cdddccdddddd")
  # "queryID","queryLen","queryStart","queryEnd","strand","refID","refLen","refStart","refEnd","numResidueMatches","lenAln","mapQ"
  alignments <- alignments %>% 
    select(queryID = X1, queryLen = X2, queryStart = X3, queryEnd = X4, strand = X5, refID = X6, refLen = X7, refStart = X8, refEnd = X9, numResidueMatches = X10, lenAln = X11, mapQ = X12) %>%
    mutate(
      #lenAlnRef = refEnd - refStart, lenAlnQuery = queryEnd - queryStart, 
      lenAlnRef = lenAln, lenAlnQuery = lenAln, # 虽然minimap2结果有refAln长度信息，但为了适应mummer处理代码
      pctIDY = numResidueMatches / lenAln * 100, 
      tmp = queryStart,
      queryStart = if_else(strand == "-", queryEnd, queryStart),
      queryEnd   = if_else(strand == "-", tmp, queryEnd), 
    ) %>% 
    select(-tmp, -refLen, -queryLen) %>% 
    filter(mapQ >= min_mq) # filter mapQ
}
...
ggsave(p_color, filename = paste(output_filename, paste(tool, "dotplot", sep = "_"), "png", sep = "."), width = plot_width, height = plot_height, units = "in", dpi = 500)
ggsave(p_color, filename = paste(output_filename, paste(tool, "dotplot", sep = "_"), "pdf", sep = "."), width = plot_width, height = plot_height, units = "in")
