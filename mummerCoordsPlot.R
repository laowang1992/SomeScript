...
p <- add_argument(parser = p, arg = "--refIDs", short = "-R", help = "a tab-separated file containing two columns, first column is IDs in reference genome, second column is names to shown in plot", type = "character", default = NULL)
p <- add_argument(parser = p, arg = "--queryIDs", short = "-Q", help = "a tab-separated file containing two columns, first column is IDs in query genome, second column is names to shown in plot", type = "character", default = NULL)
p <- add_argument(parser = p, arg = "--refLen", short = "-l", help = "a tab-separated file containing two columns, first column is IDs in reference genome, second column is chr length", default = NULL)
p <- add_argument(parser = p, arg = "--queryLen", short = "-L", help = "a tab-separated file containing two columns, first column is IDs in query genome, second column is chr length", default = NULL)
p <- add_argument(parser = p, arg = "--min-query-length", short = "-M", help = "filter queries with total alignments less than cutoff X bp", type = "integer", default = 400000)
p <- add_argument(parser = p, arg = "--min-alignment-length", short = "-m", help = "filter alignments less than cutoff X bp", type = "integer", default = 10000)
...
p <- add_argument(parser = p, arg = "--color-by", short = "-c", help = "turn on color alignments by 'direction' or 'identity', no color if not assign", type = "character", default = NULL)
...
auto_label <- function(x) {
  if_else(x < 1e6,  # 如果数字小于1百万，单位为Kb
         paste0(format(x / 1e3, scientific = FALSE), "Kb"),
         paste0(format(x / 1e6, scientific = FALSE), "Mb"))
}
p <- ggplot(alignments) + 
  ...
  scale_x_continuous(expand = c(0, 0), breaks = refLen$refBreaks, labels = refLen$refID, name = ref, 
                     sec.axis = sec_axis(~., labels = auto_label)) + 
...
p_color <- switch(
  ...
  direction = {
    ## colored by strand
    p + geom_segment(aes(x = newRefStart, xend = newRefEnd, y = newQueryStart, yend = newQueryEnd, color = strand), linewidth = 1) +
      scale_color_manual(breaks = c("+", "-"), values = c("red", "blue"), labels = c("Forward", "Reverse"), name = "Orientation", na.value = "black")
  },
  identity = {
    ## colored by identity
    p + geom_segment(aes(x = newRefStart, xend = newRefEnd, y = newQueryStart, yend = newQueryEnd, color = weighted_avg_identity), linewidth = 1) +
      scale_color_gradient(low = "blue", high = "red", name = "Identity", na.value = "black")
  },
  {
    ## no color
    p + geom_segment(aes(x = newRefStart, xend = newRefEnd, y = newQueryStart, yend = newQueryEnd), color = "black", linewidth = 1)
  }
)
